...
def _captcha_solver_funcs():
    """Ленивый импорт (во избежание циклического импорта на старте) —
    единственная РЕАЛЬНАЯ реализация капчи живёт в captcha_solver.py.
    Раньше здесь был собственный дублирующий send_captcha_to_user со своим
    отдельным captcha_storage — но реальные клики пользователя обрабатывал
    handle_captcha_answer из captcha_solver.py, который читал ДРУГОЕ,
    несвязанное хранилище. Из-за этого клик по цифре капчи никогда не
    долетал до pr gramm бота ("Капча не найдена"). Теперь используется
    только один источник правды."""
    from captcha_solver import send_captcha_to_user, get_captcha_data
    return send_captcha_to_user, get_captcha_data


async def send_captcha_to_user(msg, chat_id: int, client: TelegramClient) -> bool:
    """Отправляет капчу пользователю, но только если для этого чата ещё
    нет активной (неотвеченной) капчи — иначе при каждом новом цикле
    do_cycle пользователь получал бы повторное уведомление с фото и
    кнопками, хотя предыдущая капча ещё не решена (спам)."""
    real_send, get_data = _captcha_solver_funcs()
    existing = get_data(chat_id)
    if existing and not existing.get('answered', False):
        logging.info(f"🧩 Капча уже отправлена пользователю {chat_id}, повторно не уведомляю")
        return True
    return await real_send(msg, chat_id, client)
...
async def task_choose_callback(callback: types.CallbackQuery):
    try:
        ...
        if task_type == "bots" and not is_premium(user_id):
            await callback.answer(
                f"{PREMIUM_ICON} Тип заданий 'Боты' доступен только с премиум-подпиской",
                show_alert=True
            )
            return
...
webapp_captcha_pending: Dict[int, List[str]] = {}  # user_id -> [phone, ...]


def is_webapp_captcha_message(msg) -> bool:
    """Определяет капчу, которая открывается через WebApp-кнопку (а не
    обычными inline-кнопками с цифрами) — характерно для типа заданий
    'Просмотр постов'."""
    if not msg or not msg.buttons:
        return False
    for row in msg.buttons:
        for btn in row:
            ...
            if 'WebView' in type_name or 'WebApp' in type_name:
                return True
    return False
...
async def _handle_captcha(msg, task_type: str, user_id: int, phone: str, client) -> bool:
    ...
    if task_type == "posts" and is_webapp_captcha_message(msg):
        ...
        if phone not in pending:
            ...
            bu = bot_username_for_task.get(phone, "")
            text = (
                f"обнаружена капча в задании: Просмотр постов\n"
                ...
                for attempt in range(5):
                    try:
                        ...
                        if await client.is_user_authorized():
                            ...
                            # Не авторизован даже после успешного коннекта —
                            # это реальный логаут, а не сетевой сбой.
                            logging.error(f"❌ Сессия {phone} разлогинена")
                            break
                    except Exception as e:
                        logging.warning(
                            f"⚠️ Попытка переподключения {attempt + 1}/5 для {phone} не удалась: {e}"
                        )
