...
async def send_code(phone: str, bot_username: str) -> bool:
    ...
    # повтор вне лока, чтобы не было дедлока при рекурсии
...
def _find_row_pairs(msg):
    """
    Находит пары кнопок (подписка_url, проверить) в одном ряду.
    Именно так устроены задания в PR GRAM: каждая строка = одно задание,
    слева ссылка на подписку, справа своя кнопка "Проверить".
    """
    pairs = []
    if not msg or not msg.buttons:
        return pairs
...
async def process_channel_list(client: TelegramClient, bot_username: str, msg, phone: str):
    ...
    Каждое задание (ряд) обрабатывается независимо: своя ссылка + своя кнопка "Проверить".
    """
    try:
        ...
        while True:
            ...
            task_pairs = _find_row_pairs(msg)
            ...
            for i, (sub_btn, check_btn) in enumerate(task_pairs, 1):
                ...
                for attempt in range(1, max_attempts + 1):
                    ...
                    if "не подписан" in resp_text:
                        ...
                        # ищем актуальную кнопку "Проверить" именно в этом же ряду
                        new_pairs = _find_row_pairs(updated_msg)
                        if len(new_pairs) >= i:
